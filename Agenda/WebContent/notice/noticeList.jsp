	...
	<section>
		...
		<form action="${pageContext.request.contextPath}/NoticeController.do"
			...
			<input type="hidden" name="command" value="list"> <input
				type="hidden" name="curPage" value="1"> 총 ${map.count}개의
			게시물이 있습니다.
			<table>
				...
				<c:forEach var="row" items="${map.list}">
					<c:choose>
						<c:when test="${row.notice_delflag=='N'}">
							...
							<tr>
								<td>${row.notice_no}</td>
								<!-- 게시글 상세보기 페이지로 이동시 목록페이지에 있는 검색조건, 키워드, 현재 페이지 값을 유지하기 위해  -->
								<td><a
				...
				<tr>
					<td colspan="5">
						...
						</c:if> <!-- 이전 페이지 블록으로 이동: 현재 페이지 블럭이 1보다 크면 [이전]하이퍼링크를 화면에 출력 --> <c:if
							test="${map.boardPager.curBlock>1 }">
						...
						<!-- 다음페이지 블록으로 이동: 현재 페이지 블럭이 전체 페이지 블럭보다 작거나 같으면 [다음]하이퍼링크를 화면에 출력함 -->
						<c:if
							test="${map.boardPager.curBlock <=map.boardPager.totBlock}">
							<a href="javascript:list('${map.boardPager.nextPage }')">[다음]</a>
						</c:if>
						<!-- 끝페이지로 이동: 현재 페이지가 전체 페이지보다 작거나 같으면 [끝] 하이퍼 링크를 화면에 출력 -->
