...
public class NoticeBoardPager {
	...
	private int blockEnd; //현재 페이지를 블록의 끝번호로
	
	//생성자
	//BoardPager(레코드 갯수, 현재 페이지 번호)
	public NoticeBoardPager(int count, int curPage) {
		curBlock = 1; //현재 페이지 블록 번호
		this.curPage = curPage; //전체 페이지 갯수 계산
		setTotPage(count); //전체 페이지 갯수 계산
		setPageRange(); //
		setTotBlock();//전체 페이지 블록갯수 계산
		setBlockRange();//페이지블록의 시작, 끝번호 계산
