...
@WebServlet("/KakaoRegistController")
public class KakaoRegistController extends HttpServlet {
	private static final long serialVersionUID = 1L;
     
    public KakaoRegistController() {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//String command = (String) session.getAttribute("command");	//이걸로하면 이메일안먹음
		//String member_id = (String) session.getAttribute("member_id");
		//int member_name = (int) session.getAttribute("member_name");
		...
		if (command.equals("idcheck")) {
			...
			if(member_email_valid.equals(SHA256.getEncryptSaltFixed(member_email)))
			{
				...
				if (res) {	
					...
					registResponse("다시 회원가입 하세요", "regist/KakaoRegistForm.jsp", response);
